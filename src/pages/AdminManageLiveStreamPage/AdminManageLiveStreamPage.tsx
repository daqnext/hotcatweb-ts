...
import { ELiveStreamStatus, ILanguage } from "../../interface/interface";
...
class ManageLiveStreamPage extends React.Component<Props, State> {
    ...
    async deleteStream(streamId: number, secret: string) {
        ...
        if (response && response.status === 0) {
            const streamArray=[...this.state.data]
            for (let i = 0; i < streamArray.length; i++) {
                if (streamArray[i].id===streamId) {
                    streamArray.splice(i)
                    break
                }
            }

            this.setState({data:streamArray})
    ...
    renderForm(){
        ...
        <form className="row">
                    ...
                    <div className="col-4">
                        ...
                        <input className="form-control" placeholder="Stream Id" value={this.state.streamId} onChange={(e)=>{
                            if (e.target.value==="") {
